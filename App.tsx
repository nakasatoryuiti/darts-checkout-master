...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#111] text-gray-100 font-sans pb-10">
      {/* Header */}
      <header className="bg-dart-black border-b border-gray-800 p-3 sm:p-4 sticky top-0 z-10 shadow-lg backdrop-blur-sm bg-opacity-95">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Target className="text-dart-red w-5 h-5 sm:w-6 sm:h-6" />
            <h1 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-dart-red to-dart-green bg-clip-text text-transparent hidden sm:block">
              Checkout Master
            </h1>
            <h1 className="text-lg font-bold bg-gradient-to-r from-dart-red to-dart-green bg-clip-text text-transparent sm:hidden">
              DartsMaster
            </h1>
          </div>
          {isValidScore && (
            <div className="text-[10px] sm:text-sm text-gray-400 bg-gray-900 px-2 py-1 rounded">
              {paths.length} Ways
            </div>
          )}
        </div>
      ...
      <main className="max-w-2xl mx-auto p-3 sm:p-4 flex flex-col items-center">
        
        {/* Mode Selector */}
        <div className="w-full mb-4 sm:mb-6 p-1 bg-gray-900 rounded-lg flex gap-1 relative overflow-hidden border border-gray-800">
           <ModeButton 
        ...
        <div className="mb-4 sm:mb-6 w-full flex flex-col items-center">
            <div className="text-gray-400 text-[10px] sm:text-sm mb-1 uppercase tracking-widest font-semibold text-center">
              残りスコア ({gameMode === 'double_out' ? 'Double' : gameMode === 'master_out' ? 'Master' : 'Single'})
            </div>
            <div className={`
              text-6xl sm:text-8xl font-black tabular-nums tracking-tight transition-colors duration-300
           ...
           <div className="w-full mb-6 sm:mb-8">
             {!advice && !loadingAdvice ? (
                <button 
                  ...
                  className="w-full py-3 rounded-lg bg-gradient-to-r from-purple-900 to-indigo-900 border border-purple-700 text-purple-100 font-semibold flex items-center justify-center gap-2 hover:brightness-110 transition-all shadow-[0_0_15px_rgba(88,28,135,0.4)] text-sm sm:text-base"
                >
                ...
                <div className="bg-gray-800/50 border border-purple-500/30 rounded-lg p-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center gap-2 mb-2 text-purple-400 font-bold text-xs sm:text-sm uppercase tracking-wider">
        ...
        {isValidScore && paths.length === 0 ? (
           <div className="text-center py-10 opacity-50">
             <p className="text-lg sm:text-xl font-semibold">チェックアウトできません</p>
             <p className="text-xs sm:text-sm mt-2">（このルールでは上がれません）</p>
           </div>
        ) : (
          <div className="w-full space-y-6 sm:space-y-8">
            ...
            {groupedPaths[1].length > 0 && (
              <section>
                 <h2 className="text-dart-gold font-bold mb-3 sm:mb-4 flex items-center gap-2 text-base sm:text-lg border-b border-gray-800 pb-2">
            ...
            {groupedPaths[2].length > 0 && (
              <section>
                 <h2 className="text-dart-green font-bold mb-3 sm:mb-4 flex items-center gap-2 text-base sm:text-lg border-b border-gray-800 pb-2">
                   2本アレンジ
                 </h2>
                 <div className="grid gap-2 sm:gap-3">
            ...
            {groupedPaths[3].length > 0 && (
              <section>
                 <h2 className="text-blue-400 font-bold mb-3 sm:mb-4 flex items-center gap-2 text-base sm:text-lg border-b border-gray-800 pb-2">
                   3本アレンジ (Full House)
                 </h2>
                 <div className="grid gap-2 sm:gap-3">
...
const ModeButton: React.FC<{ active: boolean; onClick: () => void; label: string; icon: React.ReactNode }> = ({ active, onClick, label, icon }) => (
  <button
    onClick={onClick}
    className={`
      flex-1 py-2 px-1 rounded-md text-xs sm:text-sm font-bold flex items-center justify-center gap-1 sm:gap-2 transition-all touch-manipulation
      ${active 
...
const CheckoutRow: React.FC<{ path: CheckoutPath }> = ({ path }) => {
  ...
  return (
    <div className={`
      relative rounded-xl p-2 sm:p-4 flex items-center justify-between shadow-sm transition-all
      ${isUniform 
    ...
    `}>
       {isUniform && (
         <div className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2">
            <span className="bg-dart-gold text-black text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full uppercase shadow-lg">
              SAME {path.darts[0].score}
            </span>
         </div>
       )}
       <div className="flex items-center gap-1 sm:gap-4 flex-1 justify-center sm:justify-start">
          {path.darts.map((dart, i) => (
            <React.Fragment key={i}>
              <DartTag dart={dart} />
              {i < path.darts.length - 1 && (
                <div className="w-2 h-0.5 sm:w-4 sm:h-1 bg-gray-700 rounded-full hidden sm:block opacity-50"></div>
              )}
            </React.Fragment>
          ))}
       </div>
